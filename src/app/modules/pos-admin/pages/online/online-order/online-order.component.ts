...
import { ItemDiscountComponent } from '../../order/item-discount/item-discount.component';
import { ModifierPopUpComponent } from '../../order/modifier-pop-up/modifier-pop-up.component';
...
export class OnlineOrderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      if (sessionStorage.getItem('PayNowFromOnline') == 'payNow') {
        this.PayNow();
      }
    }, 2000);
  }
  ...
  OpenDiscountDialog(dialogType, content, itemIndex) {
    ...
    this.itemDiscount = this.allDiscounts.filter(item => matchedItem.discount?.includes(item.discountId));


    for (let ad = 0; ad < this.itemDiscount?.length; ad++) {
      if (this.itemDiscount[ad].discountType == 'Percentage') {
        this.itemDiscount[ad].discountedAmount = (this.PrimaryOrder.items[this.selectedItemIndex].itemTotal - AllDiscounttotal) * this.itemDiscount[ad].discountValue / 100;
      } else if (this.itemDiscount[ad].discountType == 'Amount') {
        this.itemDiscount[ad].discountedAmount = this.itemDiscount[ad].discountValue * this.PrimaryOrder.items[this.selectedItemIndex].orderQuantity;
      }
    }
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title', keyboard: false, backdrop: 'static', windowClass: 'main_add_popup add_discount_popup' }).result.then((result) => {
    }, (reason) => {
  ...
  OpenPrmocodeDialog(dialogType, content, index) {
    ...
    const selectedItemId = this.PrimaryOrder.items[this.selectedItemIndex].itemId;
    const matchedItem = this.allItems.find(item => item.id === selectedItemId);
    this.itemDiscount = this.allPromocodes.filter(item => matchedItem.discount?.includes(item.promocodeId));


    for (let ad = 0; ad < this.itemDiscount?.length; ad++) {
      if (this.itemDiscount[ad].promocodeType == 'Percentage') {
        this.itemDiscount[ad].discountedAmount = (this.PrimaryOrder.items[this.selectedItemIndex].itemTotal - AllDiscounttotal) * this.itemDiscount[ad].promocodeValue / 100;
      } else if (this.itemDiscount[ad].promocodeType == 'Amount') {
        this.itemDiscount[ad].discountedAmount = this.itemDiscount[ad].promocodeValue * this.PrimaryOrder.items[this.selectedItemIndex].orderQuantity;
      }
    }
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title', keyboard: false, backdrop: 'static', windowClass: 'main_add_popup add_discount_popup' }).result.then((result) => {
    }, (reason) => {
